...
export const addCatagory = catchAsync(async(req,res,next)=>{
  ...
  console.log(uniqueCatagory);
    const  categoryExist = await Catagory.findOne({name :uniqueCatagory,activeStatus:true})
     if(categoryExist){
...
export const editCatagory =  catchAsync( async (req, res, next) =>{
  ...
  console.log(uniqueCatagory);
    const  categoryExist = await Catagory.findOne({name :uniqueCatagory})
     ...
     const  updatedCategory=await Catagory.updateOne({_id: id,activeStatus:true},{$set:{name:uniqueCatagory}})
...
export  const deleteCatagory = catchAsync(async (req, res, next) => {
  const deletedCatagory = await Catagory.updateOne({ _id: req.params.id },{$set: {activeStatus: false}})
  if(!deleteCatagory){
  ...
  export const setApproval = catchAsync(async (req, res, next) => {
   ...
   const isApproved =  await Course.findByIdAndUpdate(req.params.id, { $set: { status: status } });
   if(!isApproved){
